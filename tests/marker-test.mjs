...
test.only("string concat", async (t) =>
  t.is(
    await expand("${{'x' + 'y'}}", {
      leftMarker: "${{",
      rightMarker: "}}",
...
test("addition", async (t) =>
  t.is(await expand("${{1 + 2}}", { ...extraMarker }), 3));
...
test("division", async (t) =>
  t.is(await expand("${{8/2}}", { ...extraMarker }), 4));
test("number", async (t) =>
...
test("replace", async (t) =>
  t.is(
    await expand("${{replace('lower','ow','12')}}", { ...extraMarker }),
    "l12er"
  ));
...
test("substring with expressions", async (t) =>
  t.is(
    await expand("${{substring('lower',1,1+2*1)}}", { ...extraMarker }),
    "ow"
  ));
...
test("array literals nested", async (t) =>
  t.deepEqual(await expand("${{[1,['a'],3]}}", { ...extraMarker }), [
    1,
    ["a"],
    3,
  ]));
